...
export default async function AdminListeningListPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Listening testlari</h1>
        <Link
          ...
          >
            <div>
              ...
              <p className="text-sm text-slate-500">
                {t.parts.length} ta Section ·{" "}
                {t.parts.reduce((acc, p) => acc + p._count.questions, 0)} ta savol
              </p>
            </div>
            <div className="flex items-center gap-4">
              <Link
                href={`/admin/listening/${t.id}/edit`}
                className="text-sm text-indigo-600 font-medium hover:text-indigo-700"
              >
                Tahrirlash
              </Link>
              <DeleteButton
                action={deleteTest.bind(null, t.id, "LISTENING")}
